...
double* get_col(uint32_t i, double** H){
	return H[i];
}

libqp_state_T libqp_gsmo_solver(double** H,
            double *diag_H,
            double *f,
            double *a,
            double b,
...
printf("diag %f %f\nf %f %f\na %f %f\nb %f\n",diag_H[0],diag_H[1],f[0],f[1],a[0],a[1],b);
printf("LB %f %f\nUB %f %f\nx %f %f\nn %i\nMaxIter %i\nTolKKT %f\n",LB[0],LB[1],UB[0],UB[1],x[0],x[1],n,MaxIter,TolKKT);
double* c1 = (double*)get_col(0,H);
double* c2 = (double*)get_col(1, H);
printf("c1 %f %f\nc2 %f %f\n",c1[0],c1[1],c2[0],c2[1]);
  double *col_u;
  double *col_v;
  double *Nabla;
  ...
  while( state.exitflag == 100 ) 
  {
    ...
    else 
    {
      ...
      col_u = (double*)get_col(u, H);
      col_v = (double*)get_col(v, H);
